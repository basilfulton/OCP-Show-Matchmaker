...
- Always encourage attending live theatre`}function S(t,i,s){const n=JSON.stringify(t,null,2),o=JSON.stringify(i,null,2),a=s.map(r=>`${r.role}: ${r.content}`).join(`
...
Return ONLY a valid JSON object — no markdown fences, no extra text:
{
...
}`}async function y(t,i,s){var a,r,l;const n=await fetch(_,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:k,max_tokens:s,messages:i})});if(!n.ok){const u=await n.json().catch(()=>({}));throw new Error(((a=u.error)==null?void 0:a.message)??`API error ${n.status}`)}return((l=(r=(await n.json()).choices[0])==null?void 0:r.message)==null?void 0:l.content)??""}async function f(t,i,s){const n=[{role:"system",content:s},...i.map(o=>({role:o.role,content:o.content}))];return y(t,n,512)}async function $(t,i,s,n){const o=S(i,s,n),a=await y(t,[{role:"user",content:o}],1024);try{return JSON.parse(a)}catch{const r=a.match(/\{[\s\S]*\}/);if(r)return JSON.parse(r[0]);throw new Error("Could not parse recommendation. Please try again.")}}const e={view:"landing",formStep:0,formAnswers:{},messages:[],shows:w,apiKey:sessionStorage.getItem("ocp_api_key")??"",isLoading:!1,matchResult:null};function d(){const t=document.getElementById("app");switch(e.view){case"landing":t.innerHTML=A();break;case"apikey":t.innerHTML=C();break;case"form":t.innerHTML=x();break;case"chat":t.innerHTML=M();break;case"result":t.innerHTML=E();break}e.isLoading&&t.insertAdjacentHTML("beforeend",L()),P()}function A(){const t=e.shows.filter(s=>s.isCurrentlyRunning),i=e.shows.filter(s=>!s.isCurrentlyRunning);return`
    <nav class="nav">
      <div>
  ...
  `}function x(){const t=p[e.formStep],i=e.formStep/p.length*100,s=t.type==="multi",n=e.formAnswers[t.id],o=s?n??[]:[n??""],a=s?o.length>0:!!n,r=e.formStep===p.length-1;return`
    <nav class="nav">
      <div>
        <div class="nav__logo">Omaha Community Playhouse</div>
        <div class="nav__subtitle">Show Matchmaker</div>
      ...
      <div class="container container--narrow">
        ...
        <div class="progress-bar">
          ...
          <div class="progress-bar__label">
            <span>${e.formStep+1} of ${p.length}</span>
        ...
        <div class="form-question">
          ...
          <div class="answer-grid">
            ${t.options.map(l=>`
              <button
                class="answer-option${o.includes(l.value)?" selected":""}"
                data-value="${l.value}"
                data-question="${t.id}"
                data-type="${t.type}"
              >
                ${l.icon?`<span class="answer-option__icon">${l.icon}</span>`:""}
                <div>
                  <div class="answer-option__text">${l.label}</div>
                  ${l.sub?`<div class="answer-option__sub">${l.sub}</div>`:""}
                </div>
              </button>
            `).join("")}
          </div>
  ...
  `}function M(){const t=e.messages.map(n=>`
    <div class="chat-message chat-message--${n.role}">
      <div class="chat-message__avatar">${n.role==="assistant"?"🎭":"👤"}</div>
      <div class="chat-message__bubble">${c(n.content)}</div>
    </div>
  `).join(""),i=e.isLoading?`
    ...
    <div class="result-screen">
      <div class="container">
        ...
        <div class="result-hero">
          <div class="result-hero__fallback">
            <div style="text-align:center;opacity:0.25;">
              ...
              <div style="font-family:var(--font-serif);font-size:1rem;letter-spacing:0.08em;">${c(t.venue)}</div>
            </div>
          </div>
          <div class="result-hero__overlay"></div>
          <div class="result-hero__content">
            <div class="result-hero__badge">✦ Your Perfect Match</div>
            <h1 class="result-hero__title">${c(t.title)}</h1>
            <p class="result-hero__dates">${c(t.dates)}</p>
          </div>
        </div>
        ...
        <div class="result-section">
          ...
          <p class="result-reason">"${c(e.matchResult.reason)}"</p>
        </div>

        <!-- About the show -->
        <div class="result-section">
          <p class="result-section__title">About the Show</p>
          <p class="result-overview">${c(t.description)}</p>

          ${t.contentAdvisory?`
            <div class="tag-list mt-16">
              <span class="tag">⚠ ${c(t.contentAdvisory)}</span>
            </div>
          `:""}
          ...
          <div class="info-grid mt-20">
            <div class="info-item">
              ...
              <span class="info-item__value">${c(t.venue)}</span>
            </div>
            <div class="info-item">
              <span class="info-item__label">Capacity</span>
              <span class="info-item__value">${c(t.venueCapacity)}</span>
            </div>
            <div class="info-item">
              <span class="info-item__label">Dates</span>
              <span class="info-item__value">${c(t.dates)}</span>
            </div>
            <div class="info-item">
          ...
          ${t.genres.length>0?`
            <div class="tag-list mt-16">
              ${t.genres.map(s=>`<span class="tag tag--gold">${c(s)}</span>`).join("")}
            </div>
          `:""}
        </div>
        ...
        ${t.specialEvents.length>0?`
          <div class="result-section">
            ...
            <div class="tag-list">
              ${t.specialEvents.map(s=>`<span class="tag tag--gold">⭐ ${c(s)}</span>`).join("")}
            </div>
          </div>
        `:""}
        ...
        ${t.accessibility.length>0?`
          <div class="result-section">
            ...
            <div class="tag-list">
              ${t.accessibility.map(s=>`<span class="tag">♿ ${c(s)}</span>`).join("")}
            </div>
          </div>
        `:""}
        ...
        <div class="result-section">
          ...
          <p class="result-overview">${c(t.venueDescription)}</p>
            ...
            <div class="compare-grid">
              ${i.map(s=>s.show?`
                <div class="compare-card">
                  <h3 class="compare-card__title">${c(s.show.title)}</h3>
                  <p class="compare-card__dates">${c(s.show.dates)}</p>
                  <p class="compare-card__note">${c(s.note)}</p>
                </div>
              `:"").join("")}
            </div>
          </div>
  ...
  `}function P(){var t,i,s,n,o;switch(e.view){case"landing":{(t=document.getElementById("start-btn"))==null||t.addEventListener("click",()=>{e.view=e.apiKey?"form":"apikey",d()});break}case"apikey":{const a=document.getElementById("api-key-input"),r=document.getElementById("save-key-btn"),l=()=>{const u=a.value.trim();u&&(e.apiKey=u,sessionStorage.setItem("ocp_api_key",u),e.view="form",d())};r==null||r.addEventListener("click",l),a==null||a.addEventListener("keydown",u=>{u.key==="Enter"&&l()});break}case"form":{document.querySelectorAll(".answer-option").forEach(a=>{a.addEventListener("click",()=>{const r=a.dataset.value,l=a.dataset.question;if(a.dataset.type==="multi"){const m=e.formAnswers[l]??[];e.formAnswers[l]=m.includes(r)?m.filter(b=>b!==r):[...m,r],d()}else e.formAnswers[l]=r,d(),setTimeout(()=>{e.formStep<p.length-1?(e.formStep++,d()):h()},320)})}),(i=document.getElementById("next-btn"))==null||i.addEventListener("click",()=>{e.formStep<p.length-1?(e.formStep++,d()):h()}),(s=document.getElementById("prev-btn"))==null||s.addEventListener("click",()=>{e.formStep>0&&(e.formStep--,d())});break}case"chat":{const a=document.getElementById("chat-input"),r=document.getElementById("chat-send");a==null||a.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(a.scrollHeight,120)+"px"}),a==null||a.addEventListener("keydown",l=>{const u=l;u.key==="Enter"&&!u.shiftKey&&(u.preventDefault(),v())}),r==null||r.addEventListener("click",()=>void v()),(n=document.getElementById("recommend-btn"))==null||n.addEventListener("click",()=>void O()),I();break}case"result":{(o=document.getElementById("start-over-btn"))==null||o.addEventListener("click",()=>{e.view="landing",e.formStep=0,e.formAnswers={},e.messages=[],e.matchResult=null,d()});break}}}async function h(){e.view="chat",e.messages=[],e.isLoading=!0,d();const t={role:"user",content:"Hi! I've answered some questions about my preferences. Can you help me find the perfect show at OCP?"};e.messages=[t];try{const i=g(e.shows,e.formAnswers),s=await f(e.apiKey,e.messages,i);e.messages=[...e.messages,{role:"assistant",content:s}]}catch(i){e.messages=[...e.messages,{role:"assistant",content:`I'm having trouble connecting right now. Please check your API key and try again. (${i.message})`}]}e.isLoading=!1,d()}async function v(){const t=document.getElementById("chat-input"),i=t==null?void 0:t.value.trim();if(!(!i||e.isLoading)){e.messages=[...e.messages,{role:"user",content:i}],e.isLoading=!0,d();try{const s=g(e.shows,e.formAnswers),n=await f(e.apiKey,e.messages,s);e.messages=[...e.messages,{role:"assistant",content:n}]}catch(s){e.messages=[...e.messages,{role:"assistant",content:`Something went wrong: ${s.message}`}]}e.isLoading=!1,d()}}async function O(){e.isLoading=!0,d();try{const t=await $(e.apiKey,e.shows,e.formAnswers,e.messages);e.matchResult=t,e.view="result"}catch(t){e.messages=[...e.messages,{role:"assistant",content:`I had trouble generating your recommendation. ${t.message} Please try again.`}]}e.isLoading=!1,d()}function I(){requestAnimationFrame(()=>{const t=document.getElementById("chat-messages");t&&(t.scrollTop=t.scrollHeight)})}function c(t){const i=document.createElement("div");return i.appendChild(document.createTextNode(t)),i.innerHTML}d();
