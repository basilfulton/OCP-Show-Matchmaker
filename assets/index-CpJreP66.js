...
- Always encourage attending live theatre`}function S(t,a,o){const s=JSON.stringify(t,null,2),n=JSON.stringify(a,null,2),i=o.map(r=>`${r.role}: ${r.content}`).join(`
...
Return ONLY a valid JSON object — no markdown fences, no extra text:
{
...
}`}async function y(t,a,o){var i,r,c;const s=await fetch(_,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:k,max_tokens:o,messages:a})});if(!s.ok){const u=await s.json().catch(()=>({}));throw new Error(((i=u.error)==null?void 0:i.message)??`API error ${s.status}`)}return((c=(r=(await s.json()).choices[0])==null?void 0:r.message)==null?void 0:c.content)??""}async function f(t,a,o){const s=[{role:"system",content:o},...a.map(n=>({role:n.role,content:n.content}))];return y(t,s,512)}async function $(t,a,o,s){const n=S(a,o,s),i=await y(t,[{role:"user",content:n}],1024);try{return JSON.parse(i)}catch{const r=i.match(/\{[\s\S]*\}/);if(r)return JSON.parse(r[0]);throw new Error("Could not parse recommendation. Please try again.")}}const e={view:"landing",formStep:0,formAnswers:{},messages:[],shows:w,apiKey:sessionStorage.getItem("ocp_api_key")??"",isLoading:!1,matchResult:null};function d(){const t=document.getElementById("app");switch(e.view){case"landing":t.innerHTML=A();break;case"apikey":t.innerHTML=C();break;case"form":t.innerHTML=x();break;case"chat":t.innerHTML=M();break;case"result":t.innerHTML=E();break}e.isLoading&&e.view!=="chat"&&t.insertAdjacentHTML("beforeend",L()),O()}function A(){return e.shows.filter(a=>a.isCurrentlyRunning),e.shows.filter(a=>!a.isCurrentlyRunning),`
    <nav class="nav">
      <div>
  ...
  `}function x(){const t=p[e.formStep],a=(e.formStep+1)/p.length*100,o=t.type==="multi",s=e.formAnswers[t.id],n=o?s??[]:[s??""],i=o?n.length>0:!!s,r=e.formStep===p.length-1;return`
    <nav class="nav">
      <div>
        <div class="nav__logo">Omaha Community Playhouse</div>
        <div class="nav__subtitle">Show Matchmaker</div>
      ...
      <div class="container container--narrow">
        ...
        <div class="progress-bar">
          ...
          <div class="progress-bar__label">
            <span>Question ${e.formStep+1} of ${p.length}</span>
            <span class="progress-bar__type">${o?"Select all that apply":"Select one"}</span>
          </div>
        </div>
  ...
  `}function M(){const t=e.messages.map((s,n)=>`
    <div class="chat-message chat-message--${s.role}" style="animation-delay:${n*30}ms">
      ...
      <div class="chat-message__bubble">${s.role==="assistant"?T(s.content):l(s.content)}</div>
    </div>
  `).join(""),a=e.isLoading?`
  ...
  `:"",o=e.messages.length>=3&&!e.isLoading;return`
    <nav class="nav">
      <div>
        <div class="nav__logo">Omaha Community Playhouse</div>
        <div class="nav__subtitle">Show Matchmaker</div>
  ...
  `}function O(){var t,a,o,s,n;switch(e.view){case"landing":{(t=document.getElementById("start-btn"))==null||t.addEventListener("click",()=>{e.view=e.apiKey?"form":"apikey",d()});break}case"apikey":{const i=document.getElementById("api-key-input"),r=document.getElementById("save-key-btn"),c=()=>{const u=i.value.trim();u&&(e.apiKey=u,sessionStorage.setItem("ocp_api_key",u),e.view="form",d())};r==null||r.addEventListener("click",c),i==null||i.addEventListener("keydown",u=>{u.key==="Enter"&&c()});break}case"form":{document.querySelectorAll(".answer-option").forEach(i=>{i.addEventListener("click",()=>{const r=i.dataset.value,c=i.dataset.question;if(i.dataset.type==="multi"){const m=e.formAnswers[c]??[];e.formAnswers[c]=m.includes(r)?m.filter(b=>b!==r):[...m,r],d()}else e.formAnswers[c]=r,d(),setTimeout(()=>{e.formStep<p.length-1?(e.formStep++,d()):h()},320)})}),(a=document.getElementById("next-btn"))==null||a.addEventListener("click",()=>{e.formStep<p.length-1?(e.formStep++,d()):h()}),(o=document.getElementById("prev-btn"))==null||o.addEventListener("click",()=>{e.formStep>0&&(e.formStep--,d())});break}case"chat":{const i=document.getElementById("chat-input"),r=document.getElementById("chat-send");i==null||i.addEventListener("input",()=>{i.style.height="auto",i.style.height=Math.min(i.scrollHeight,120)+"px"}),i==null||i.addEventListener("keydown",c=>{const u=c;u.key==="Enter"&&!u.shiftKey&&(u.preventDefault(),v())}),r==null||r.addEventListener("click",()=>void v()),(s=document.getElementById("recommend-btn"))==null||s.addEventListener("click",()=>void P()),I();break}case"result":{(n=document.getElementById("start-over-btn"))==null||n.addEventListener("click",()=>{e.view="landing",e.formStep=0,e.formAnswers={},e.messages=[],e.matchResult=null,d()});break}}}async function h(){e.view="chat",e.messages=[],e.isLoading=!0,d();const t={role:"user",content:"Hi! I've answered some questions about my preferences. Can you help me find the perfect show at OCP?"};e.messages=[t];try{const a=g(e.shows,e.formAnswers),o=await f(e.apiKey,e.messages,a);e.messages=[...e.messages,{role:"assistant",content:o}]}catch(a){e.messages=[...e.messages,{role:"assistant",content:`I'm having trouble connecting. Please check your API key and try again. (${a.message})`}]}e.isLoading=!1,d()}async function v(){const t=document.getElementById("chat-input"),a=t==null?void 0:t.value.trim();if(!(!a||e.isLoading)){e.messages=[...e.messages,{role:"user",content:a}],e.isLoading=!0,d();try{const o=g(e.shows,e.formAnswers),s=await f(e.apiKey,e.messages,o);e.messages=[...e.messages,{role:"assistant",content:s}]}catch(o){e.messages=[...e.messages,{role:"assistant",content:`Something went wrong: ${o.message}`}]}e.isLoading=!1,d()}}async function P(){e.isLoading=!0,d();try{const t=await $(e.apiKey,e.shows,e.formAnswers,e.messages);e.matchResult=t,e.view="result"}catch(t){e.messages=[...e.messages,{role:"assistant",content:`I had trouble generating your recommendation. ${t.message} Please try again.`}]}e.isLoading=!1,d()}function I(){requestAnimationFrame(()=>{const t=document.getElementById("chat-messages");t&&(t.scrollTop=t.scrollHeight)})}function l(t){const a=document.createElement("div");return a.appendChild(document.createTextNode(t)),a.innerHTML}function T(t){return l(t).replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\*(.*?)\*/g,"<em>$1</em>").replace(/\n\n+/g,'</p><p class="chat-p">').replace(/\n/g,"<br>").replace(/^(.+)/,'<p class="chat-p">$1</p>')}d();
