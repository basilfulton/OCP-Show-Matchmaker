...
- Always encourage attending live theatre`}function S(t,a,r){const s=JSON.stringify(t,null,2),n=JSON.stringify(a,null,2),i=r.map(o=>`${o.role}: ${o.content}`).join(`
...
Return ONLY a valid JSON object — no markdown fences, no extra text:
{
...
}`}async function f(t,a,r){var i,o,c;const s=await fetch(_,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({model:k,max_tokens:r,messages:a})});if(!s.ok){const d=await s.json().catch(()=>({}));throw new Error(((i=d.error)==null?void 0:i.message)??`API error ${s.status}`)}return((c=(o=(await s.json()).choices[0])==null?void 0:o.message)==null?void 0:c.content)??""}async function b(t,a,r){const s=[{role:"system",content:r},...a.map(n=>({role:n.role,content:n.content}))];return f(t,s,512)}async function $(t,a,r,s){const n=S(a,r,s),i=await f(t,[{role:"user",content:n}],1024);try{return JSON.parse(i)}catch{const o=i.match(/\{[\s\S]*\}/);if(o)return JSON.parse(o[0]);throw new Error("Could not parse recommendation. Please try again.")}}const e={view:"landing",formStep:0,formAnswers:{},messages:[],shows:w,apiKey:sessionStorage.getItem("ocp_api_key")??"",isLoading:!1,matchResult:null};function u(){const t=document.getElementById("app");switch(e.view){case"landing":t.innerHTML=A();break;case"apikey":t.innerHTML=C();break;case"form":t.innerHTML=x();break;case"chat":t.innerHTML=M();break;case"result":t.innerHTML=E();break}e.isLoading&&e.view!=="chat"&&t.insertAdjacentHTML("beforeend",L()),O()}function A(){return e.shows.filter(a=>a.isCurrentlyRunning),e.shows.filter(a=>!a.isCurrentlyRunning),`
    <nav class="nav">
      <div>
  ...
  `}function x(){const t=m[e.formStep],a=(e.formStep+1)/m.length*100,r=t.type==="multi",s=e.formAnswers[t.id],n=r?s??[]:[s??""],i=r?n.length>0:!!s,o=e.formStep===m.length-1;return`
    <nav class="nav">
      <div>
        <div class="nav__logo">Omaha Community Playhouse</div>
        <div class="nav__subtitle">Show Matchmaker</div>
      ...
      <div class="container container--narrow">
        ...
        <div class="progress-bar">
          ...
          <div class="progress-bar__label">
            <span>Question ${e.formStep+1} of ${m.length}</span>
  ...
  `}function O(){var t,a,r,s,n;switch(e.view){case"landing":{(t=document.getElementById("start-btn"))==null||t.addEventListener("click",()=>{e.view=e.apiKey?"form":"apikey",u()});break}case"apikey":{const i=document.getElementById("api-key-input"),o=document.getElementById("save-key-btn"),c=()=>{const d=i.value.trim();d&&(e.apiKey=d,sessionStorage.setItem("ocp_api_key",d),e.view="form",u())};o==null||o.addEventListener("click",c),i==null||i.addEventListener("keydown",d=>{d.key==="Enter"&&c()});break}case"form":{document.querySelectorAll(".answer-option").forEach(i=>{i.addEventListener("click",()=>{const o=i.dataset.value,c=i.dataset.question;if(i.dataset.type==="multi"){const p=e.formAnswers[c]??[];e.formAnswers[c]=p.includes(o)?p.filter(h=>h!==o):[...p,o],u()}else{document.querySelectorAll(".answer-option").forEach(h=>{h.classList.toggle("selected",h.dataset.value===o)});const p=document.getElementById("next-btn");p&&p.removeAttribute("disabled"),e.formAnswers[c]=o,setTimeout(()=>{e.formStep<m.length-1?(e.formStep++,u()):v()},320)}})}),(a=document.getElementById("next-btn"))==null||a.addEventListener("click",()=>{e.formStep<m.length-1?(e.formStep++,u()):v()}),(r=document.getElementById("prev-btn"))==null||r.addEventListener("click",()=>{e.formStep>0&&(e.formStep--,u())});break}case"chat":{const i=document.getElementById("chat-input"),o=document.getElementById("chat-send");i==null||i.addEventListener("input",()=>{i.style.height="auto",i.style.height=Math.min(i.scrollHeight,120)+"px"}),i==null||i.addEventListener("keydown",c=>{const d=c;d.key==="Enter"&&!d.shiftKey&&(d.preventDefault(),g())}),o==null||o.addEventListener("click",()=>void g()),(s=document.getElementById("recommend-btn"))==null||s.addEventListener("click",()=>void P()),I();break}case"result":{(n=document.getElementById("start-over-btn"))==null||n.addEventListener("click",()=>{e.view="landing",e.formStep=0,e.formAnswers={},e.messages=[],e.matchResult=null,u()});break}}}async function v(){e.view="chat",e.messages=[],e.isLoading=!0,u();const t={role:"user",content:"Hi! I've answered some questions about my preferences. Can you help me find the perfect show at OCP?"};e.messages=[t];try{const a=y(e.shows,e.formAnswers),r=await b(e.apiKey,e.messages,a);e.messages=[...e.messages,{role:"assistant",content:r}]}catch(a){e.messages=[...e.messages,{role:"assistant",content:`I'm having trouble connecting. Please check your API key and try again. (${a.message})`}]}e.isLoading=!1,u()}async function g(){const t=document.getElementById("chat-input"),a=t==null?void 0:t.value.trim();if(!(!a||e.isLoading)){e.messages=[...e.messages,{role:"user",content:a}],e.isLoading=!0,u();try{const r=y(e.shows,e.formAnswers),s=await b(e.apiKey,e.messages,r);e.messages=[...e.messages,{role:"assistant",content:s}]}catch(r){e.messages=[...e.messages,{role:"assistant",content:`Something went wrong: ${r.message}`}]}e.isLoading=!1,u()}}async function P(){e.isLoading=!0,u();try{const t=await $(e.apiKey,e.shows,e.formAnswers,e.messages);e.matchResult=t,e.view="result"}catch(t){e.messages=[...e.messages,{role:"assistant",content:`I had trouble generating your recommendation. ${t.message} Please try again.`}]}e.isLoading=!1,u()}function I(){requestAnimationFrame(()=>{const t=document.getElementById("chat-messages");t&&(t.scrollTop=t.scrollHeight)})}function l(t){const a=document.createElement("div");return a.appendChild(document.createTextNode(t)),a.innerHTML}function T(t){return l(t).replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/\*(.*?)\*/g,"<em>$1</em>").replace(/\n\n+/g,'</p><p class="chat-p">').replace(/\n/g,"<br>").replace(/^(.+)/,'<p class="chat-p">$1</p>')}u();
